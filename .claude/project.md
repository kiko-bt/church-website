...
- the per-book files
  → the source of truth for verse text; one book, one locale, one file

- manifest.json
  → read by `generateStaticParams` so routing never loads verse text

- search/<locale>.json
  → Fuse.js index for client-side search, lazy-loaded in the browser

The per-book files are hand-owned and committed. The derived files are
regenerated by `npm run bible:build` and must never be hand-edited. Nothing here
is fetched from Sanity or from an API at runtime.
...
│   │   ├── bible/
│   │   │   ├── bible.types.ts
│   │   │   ├── bible.constants.ts       # BIBLE_CANON — the fixed book order
│   │   │   ├── bible.display-names.ts   # the ONLY definition of book names
│   │   │   ├── bible.schema.ts
│   │   │   ├── bible.data.ts
│   │   │   ├── bible.reference.ts
...
│   ├── data/
│   │   └── bible/
│   │       ├── mk/<bookId>.json        # verse text (hand-owned)
│   │       ├── en/<bookId>.json        # verse text (hand-owned)
